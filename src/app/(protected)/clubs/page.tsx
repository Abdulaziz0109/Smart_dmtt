...
export default async function ClubsPage() {
  ...
  async function enrollChild(formData: FormData) {
    ...
    if (!child || !club || child.kindergartenId !== club.kindergartenId || !club.isActive) return;
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold">To'garaklar</h1>

      {canManageClubs ? (
        <form action={createClub} className="bg-white p-4 rounded shadow grid grid-cols-2 gap-2">
          <input name="name" placeholder="Nomi" className="border p-2 rounded" required />
          <input name="description" placeholder="Tavsif" className="border p-2 rounded" required />
          <input name="monthlyPrice" placeholder="Narx" className="border p-2 rounded" required />
          <input name="teacherName" placeholder="O'qituvchi" className="border p-2 rounded" required />
          <input name="capacity" placeholder="Sig'im" className="border p-2 rounded" required />
          <input name="schedule" placeholder="Jadval" className="border p-2 rounded" required />

          {isAdmin ? (
            <select name="kindergartenId" className="border p-2 rounded col-span-2" required>
          ...
          <button className="bg-blue-600 text-white rounded px-4 py-2 col-span-2">Saqlash</button>
        </form>
      ) : null}
          ...
          <tbody>
            {clubs.map((c) => (
              <tr key={c.id} className="border-b">
                ...
                <td>
                  {session.user.role === "parent" ? (
                    <form action={enrollChild} className="flex gap-2">
                      <input type="hidden" name="clubId" value={c.id} />
                      <select name="childId" className="border rounded p-1" required>
                        {parentProfile?.children
                          .filter((ch) => ch.kindergartenId === c.kindergartenId)
                          .map((ch) => (
                      ...
                      <button className="bg-green-600 text-white rounded px-2">Yozilish</button>
                    </form>
                  ) : null}
                </td>
              </tr>
