...
export async function POST(request: Request) {
  ...
  if (!file) return NextResponse.redirect(new URL("/transactions?error=1", request.url));
  ...
  if (errors.length > 0 && imported === 0) return NextResponse.redirect(new URL("/transactions?error=1", request.url));
