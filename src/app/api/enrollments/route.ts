...
export async function POST(request: Request) {
  ...
  if (!club || !club.isActive || club.kindergartenId !== child.kindergartenId) {
    return NextResponse.json({ error: "Invalid club" }, { status: 400 });
  }

  const enrollment = await prisma.enrollment.upsert({
    where: { childId_clubId: { childId: child.id, clubId: club.id } },
    update: { status: "active", endDate: null },
    create: { childId: child.id, clubId: club.id, startDate: new Date(), status: "active" }
  });
