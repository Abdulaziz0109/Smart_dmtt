...
export async function POST(request: Request) {
  ...
  if (!invoice || invoice.status !== "unpaid") return NextResponse.json({ error: "Invoice unavailable" }, { status: 400 });
  if (session.user.role === "parent" && invoice.parent.userId !== session.user.id) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

  const provider = getProvider(parsed.data.provider);
  const amount = Number(invoice.amount);
  const result = await provider.pay(invoice.id, amount);
  if (!result.success) return NextResponse.json({ error: "Payment failed" }, { status: 400 });

  await prisma.$transaction([
    prisma.invoice.update({ where: { id: invoice.id }, data: { status: "paid" } }),
    prisma.payment.create({ data: { invoiceId: invoice.id, amount, provider: parsed.data.provider, transactionReference: result.reference, paidAt: new Date(), status: "succeeded" } })
  ]);
